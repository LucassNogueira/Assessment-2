...
const canWeDeliver = (zipcode) => {
  if (deliveryAreaZipCodes.includes(zipcode) !== true) {
    console.log("we do not deliver there sorry!");
  } else {
    console.log("we can take it!");
  }
};
// canWeDeliver(85204);
...
const canWeDeliverTwo = (zipcode) => {
  for (let i = 0; i < deliveryAreaZipCodes.length; i++) {
    if (deliveryAreaZipCodes[i] !== zipcode) {
      return "not taking it";
    } else {
      return "we can take it!";
    }
  }
};

// console.log(canWeDeliverTwo(824));
...
const newDealTitle = deals[0].title.replace("15", "10");
deals[0].title = newDealTitle;
// console.log(newDealDesc);
// console.log(deals[0]);
...
const newDealDesc = deals[1].desc.replace("March", "April").trim();
deals[1] = newDealDesc;
console.log(newDealDesc);
